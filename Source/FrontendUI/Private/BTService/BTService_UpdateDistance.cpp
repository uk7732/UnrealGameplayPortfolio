...
#include "BTService/BTService_UpdateDistance.h"
	...
	if (CurrentFollowTime >= 10.f || !BB->GetValueAsBool("SkillInit"))
	{
		...
		if (SkillComp)
		{
			...
			for (const auto& it : SkillDatas)
			{
				...
				if (BestSkillData.SkillRange < it.SkillRange)
				{
					BestSkillData = it;
				}
				else if (BestSkillData.SkillRange == it.SkillRange)
				{
			...
			if (!BestSkillData.TID.IsNone())
			{
				...
				BB->SetValueAsFloat("AttackRange", FMath::Min(BestSkillData.SkillRange, Data.AttackRange));
